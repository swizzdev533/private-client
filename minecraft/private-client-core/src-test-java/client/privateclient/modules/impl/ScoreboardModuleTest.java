...
public final class ScoreboardModuleTest {
    @Test
    public void registersAndPassesSafeModulePolicy() throws Exception {
        ...
        ScoreboardModule module = new ScoreboardModule(context);

        SafeModulePolicy.verify(module);
        assertEquals("scoreboardmod", module.getId());
        assertEquals("Scoreboard Customization", module.getName());
        assertEquals(ModuleCategory.INFORMATIONAL, module.getCategory());
        assertTrue(module.isDefaultEnabled());

        ModuleRegistry registry = new ModuleRegistry(bus);
        registry.register(module);
        registry.enable("scoreboardmod");
